...
class QRAgent(distAgent):
	...
	def _build_model(self):
		...
		for i in range(self.action_size):
			outputs.append(Dense(self.N, activation='linear')(main_hidden2))
	...
	def _reward_scaling(self,state):
		if self.reward_scaling:
			if self.twap_scaling:
				...
				return state[0][0][0] - 0.001 * state[0][0][1]
			else:
				if self.market_data_size > 0:
					return state[0][0][0] / 2 + 0.5
				else:
					return state[0][0] / 2 + 0.5

	
		return 0

	# This function needs to be rolled into predict_action
	def predict_quantiles(self,state,target = False, scale_reward = True):
		#state_action = self._process_state_action(state,action_index)
		if scale_reward:
			result_scaling_factor = self._reward_scaling(state)
		else:
			result_scaling_factor = 0
		
		if DEBUG:
			print("predict state action", state)
		#state_action.shape = (1,len(state_action))
		#print(self.target_model.predict(state))
		if self.C > 0 and target:
			return [x + result_scaling_factor for x in self.target_model.predict(state)]
		return [x + result_scaling_factor for x in np.array(self.model.predict(state))]


	# predict function could be moved to distAgent and transitioned to np
	def predict(self,state,target = False,above_med = False):
		predict  = self.predict_quantiles(state,target = target)
		#print("state", state,"raw quantile array",predict)
		...
		if above_med:
			...
			#return np.add.reduce(self.qi * predict,1)
		#print(np.add.reduce(self.qi * predict,2))
		predict = np.add.reduce(self.qi * predict,2)
		#print("state", state,"reduced quantile array",predict)
		return np.reshape(predict,(1,len(predict)))
		...
		if not done:
			...
			if horizon > 1 and mem_index < (self.memory.size - 1):
				...
				if next_action_index == action1:
					...
					tree_success = True
			if not tree_success:
				predict = self.predict_quantiles(next_state,target = True, scale_reward = False)[next_action_index][0]
		else:
	...
	def fit(self,state, action_index, reward, next_state, done,mem_index = -1):
		...
		target = self.project(reward,next_state,done,self.tree_n,mem_index) - self._reward_scaling(state)
		#print(target)
		target_f = self.predict_quantiles(state,target = False,scale_reward = False)
	...
	def variance(self,state):
		predict = np.array(self.predict_quantiles(state,target = False))
		res = np.sum(np.power(predict,2), axis = 2).flatten()
		#print("res before",res)
		#print("00",np.power(self.predict_action(state,0,above_med = self.optimisticUCB),2))
		val_predict = self.predict(state)
		res = res - val_predict**2
		#print("res after",res)
