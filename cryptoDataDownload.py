...
if download_orderbook:
	dfO = q(f'select time, bid1, bidSize1, ask1, askSize1 from orderBook where date=2019.06.{day}, (bid1 <> prev bid1) or (ask1 <> prev ask1) or (bidSize1 <> prev bidSize1) or (askSize1 <> prev askSize1)')
	print(dfO.head())
	dfO.to_csv(f"2019_06_{day}BTX_O.csv")
	print("Orders done")
dfT = q(f'select time, side, size, price from trade where date=2019.06.{day}')

print(dfT.head())
dfT.to_csv(f"2019_06_{day}BTX_TF.csv")
